  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-4 space-y-10">
            ...
            {KROWN_VIBES.map((vibe, i) => (
              ...
              >
                <Link href={`/cafes?vibe=${vibe.id}`}>
                  <div className="flex-shrink-0 flex flex-col items-center gap-3 px-6 py-5 bg-[#1E1E1E] border border-[#2A2A2A] rounded-2xl hover:border-[#800020]/30 transition-all min-w-[120px]">
                    <span className="text-3xl">{vibe.emoji}</span>
                    <span className="text-xs font-medium text-white/70 text-center whitespace-nowrap">{vibe.label}</span>
                  </div>
                </Link>
              </motion.div>
            ))}
      ...
      <SectionReveal delay={0.1}>
        <section>
          <h2 className="font-playfair text-2xl font-bold text-white mb-6">KROWN Stories</h2>
